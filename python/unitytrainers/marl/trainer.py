...
from unitytrainers.marl.models import MARLModel
from unitytrainers.trainer import UnityTrainerException, Trainer

logger = logging.getLogger("unityagents")


class MARLTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed,model_path):
        ...
        super(MARLTrainer, self).__init__(sess, env, brain_name, trainer_parameters, training)
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            self.model = MARLModel(env.brains[brain_name],
    ...
    def take_action(self, all_brain_info: AllBrainInfo):
        ...
        team_obs_feed = np.zeros([agent_cnt,self.model.team_obs.shape[1].value])

        for i in range(agent_cnt):
            team_obs_feed[i][:] = curr_brain_info.vector_observations.flatten()


        feed_dict[self.model.team_obs] = team_obs_feed
        ...
        for agent_id in next_info.agents:
            ...
            if stored_info is None:
                ...
                if not stored_info.local_done[idx]:
                    ...
                    self.training_buffer[agent_id]['team_vector_obs'].append(stored_info.vector_observations)
        ...
        for l in range(len(info.agents)):
            ...
            if ((info.local_done[l] or len(agent_actions) > self.trainer_parameters['time_horizon'])
                ...
                if info.local_done[l] and not info.max_reached[l]:
                    ...
                    feed_dict[self.model.team_obs] = np.tile(bootstrapping_info.vector_observations.flatten(),[agent_cnt,1])
    ...
    def update_model(self):
        ...
        for k in range(num_epoch):
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                if self.use_states:
                    if self.is_continuous_observation:
                        feed_dict[self.model.team_obs] = np.array(
                            _buffer['team_vector_obs'][start:end]).reshape(
                            [-1,self.model.team_obs.shape[1].value])

                    else:
                        feed_dict[self.model.team_obs[i]] = np.array(
                               _buffer['team_vector_obs'][start:end]).reshape([-1,self.model.team_obs.shape[1].value])
