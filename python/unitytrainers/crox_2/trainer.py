...
from unitytrainers.crox_2.models import CROX2Model
from unitytrainers.trainer import UnityTrainerException, Trainer

logger = logging.getLogger("unityagents")


class CROX2Trainer(Trainer):
    """The PPOTrainer is an implementation of the PPO algorythm."""

    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed,model_path):
        """
        ...
        super(CROX2Trainer, self).__init__(sess, env, brain_name, trainer_parameters, training)
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            self.model:CROX2Model = CROX2Model(env.brains[brain_name],
                                  ...
                                  m_size=self.m_size,agent_cnt=int(trainer_parameters['agent_count']))
    ...
    def take_action(self, all_brain_info: AllBrainInfo):
        ...
        steps = self.get_step
        curr_brain_info:BrainInfo = all_brain_info[self.brain_name]

        self.model.create_model(curr_brain_info,self.sess,self.model_path)
        ...
        agent_cnt = len(curr_brain_info.agents)


        crox = np.zeros(shape=[agent_cnt,self.model.crox_obs.shape.dims[1],self.model.crox_obs.shape.dims[2],self.model.crox_obs.shape.dims[3]])
        temp_0 = np.dot(curr_brain_info.vector_observations,curr_brain_info.vector_observations.T)
        temp_1 = np.dot(curr_brain_info.vector_observations.T,curr_brain_info.vector_observations)
        

        for i in range(agent_cnt):
            crox[i,0:temp_0.shape[0],0:temp_0.shape[1],0] = temp_0
            crox[i,0:temp_1.shape[0],0:temp_1.shape[1],1] = temp_1


        feed_dict[self.model.crox_obs] = crox
        ...
        for agent_id in next_info.agents:
            ...
            if stored_info is None:
                ...
                if not stored_info.local_done[idx]:
                    ...
                    self.training_buffer[agent_id]['team_actions'].append(actions)

                    crox = np.zeros(shape=[self.model.crox_obs.shape.dims[1],self.model.crox_obs.shape.dims[2],self.model.crox_obs.shape.dims[3]])

                    temp = np.dot(stored_info.vector_observations,stored_info.vector_observations.T)
                    crox[0:temp.shape[0],0:temp.shape[1],0] = temp

                    temp = np.dot(stored_info.vector_observations.T,stored_info.vector_observations)
                    crox[0:temp.shape[0],0:temp.shape[1],1] = temp


                    self.training_buffer[agent_id]['team_vector_obs'].append(crox)
                    self.training_buffer[agent_id]['team_rewards'].append(np.mean(next_info.rewards,0))
        ...
        for l in range(len(info.agents)):
            ...
            if ((info.local_done[l] or len(agent_actions) > self.trainer_parameters['time_horizon'])
                ...
                if info.local_done[l] and not info.max_reached[l]:
                    ...
                    agent_cnt = len(bootstrapping_info.agents)
                    crox = np.zeros(shape=[agent_cnt,self.model.crox_obs.shape.dims[1],self.model.crox_obs.shape.dims[2],self.model.crox_obs.shape.dims[3]])

                    temp_0 = np.dot(bootstrapping_info.vector_observations,bootstrapping_info.vector_observations.T)
                    temp_1 = np.dot(bootstrapping_info.vector_observations.T,bootstrapping_info.vector_observations)
    ...
    def update_model(self):
        ...
        for k in range(num_epoch):
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                if self.use_states:
                    feed_dict[self.model.crox_obs] = np.array(_buffer['team_vector_obs'][start:end]).reshape([-1,self.model.crox_obs.shape.dims[1].value,self.model.crox_obs.shape.dims[2].value,2])
