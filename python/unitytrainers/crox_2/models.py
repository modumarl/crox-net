...
class CROX2Model(LearningModel):
    ...
    def create_model(self,agent_cnt,sess,model_path):
        ...
        if self.brain.vector_action_space_type == "continuous":
            self.create_crox_cc_actor_critic(agent_cnt,self.h_size, self.num_layers)
            self.entropy = tf.ones_like(tf.reshape(self.value, [-1])) * self.entropy
        else:
            self.create_crox_dc_actor_critic(agent_cnt,self.h_size, self.num_layers)
    ...
    def create_crox_dc_actor_critic(self,agent_cnt ,h_size, num_layers):
        num_streams = 1
        ...
        hidden_team = self.create_crox_team_obs(agent_cnt,h_size,num_layers)
        hidden = tf.concat([hidden, hidden_team], axis=1)
    ...
    def create_crox_cc_actor_critic(self,agent_cnt, h_size, num_layers):
        num_streams = 2
        hidden_streams = self.create_new_obs(num_streams, h_size, num_layers)
        ...
        hidden_value = tf.concat([hidden_value,self.vector_in, hidden_team], axis=1)
        ...
        #hidden_value = tf.concat([hidden_value,self.vector_in, c_layers.flatten(self.output),self.team_obs ], axis=1)
    ...
    def create_crox_team_obs(self,agent_cnt, h_size, num_layers):
        ...
        size = self.brain.vector_observation_space_size*self.brain.num_stacked_vector_observations
        ...
        self.crox_obs = tf.placeholder(dtype=tf.float32,shape=[None,size,size,2],name = "crox_obs")
   
        conv1 = tf.layers.conv2d(self.crox_obs, 16, kernel_size=[filter_size, filter_size], strides=[filter_size/2, filter_size/2],
                                 activation=tf.nn.elu)
        conv2 = tf.layers.conv2d(conv1, 32, kernel_size=[filter_size/2, filter_size/2], strides=[filter_size/4, filter_size/4],
                                 activation=tf.nn.elu)
        hidden = c_layers.flatten(conv2)

        for j in range(num_layers):
            hidden = tf.layers.dense(hidden, h_size, use_bias=False, activation=tf.nn.elu)
        return hidden



    def create_crox_team_obs_0(self,agent_cnt, h_size, num_layers):
        ...
        self.crox_obs_0 = tf.placeholder(dtype=tf.float32,shape=[None,size,size,1],name = "crox_obs_0")
    ...
    def create_crox_team_obs_1(self,agent_cnt, h_size, num_layers):
        ...
        self.crox_obs_1 = tf.placeholder(dtype=tf.float32,shape=[None,size,size,1],name = "crox_obs_1")
