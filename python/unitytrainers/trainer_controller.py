...
from unitytrainers.marl.trainer import MARLTrainer
from unitytrainers.crox.trainer import CROXTrainer
from unitytrainers.crox_2.trainer import CROX2Trainer
        ...
        for brain_name in self.env.external_brain_names:
            if trainer_parameters_dict[brain_name]['trainer'] == "imitation":
                ...
                self.trainers[brain_name] = PPOTrainer(sess, self.env, brain_name, trainer_parameters_dict[brain_name],
                                                       self.train_model, self.seed)
            elif trainer_parameters_dict[brain_name]['trainer'] == "marl":
                model_path = "";
                if self.load_model:
                     model_path = self.model_path
               
                self.trainers[brain_name] = MARLTrainer(sess, self.env, brain_name, trainer_parameters_dict[brain_name],
                                        self.train_model,self.seed, "")
            elif trainer_parameters_dict[brain_name]['trainer'] == "crox":
                model_path = "";
                if self.load_model:
                     model_path = self.model_path
               
                self.trainers[brain_name] = CROXTrainer(sess, self.env, brain_name, trainer_parameters_dict[brain_name],
                                        self.train_model,self.seed, "")

            elif trainer_parameters_dict[brain_name]['trainer'] == "crox2":
                model_path = "";
                if self.load_model:
                     model_path = self.model_path
               
                self.trainers[brain_name] = CROX2Trainer(sess, self.env, brain_name, trainer_parameters_dict[brain_name],
                                        self.train_model,self.seed, "")
